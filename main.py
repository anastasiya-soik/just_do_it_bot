...
AI_SYSTEM_PROMPT = (
    ...
    "Говоришь на «ты», как близкий друг который верит в человека. "
    ...
    "ЯЗЫК ОТВЕТА — ТОЛЬКО РУССКИЙ, без слов на украинском, английском или любом другом языке."
)
    ...
    while True:
        ...
        try:
            ...
            if not item:
                continue
            _, payload = item
            task_id, prompt = payload.decode().split("\x00", 1)
...
ANIMAL_FACT_SYSTEM_PROMPT = (
    ...
    "Пиши живо, без канцелярита и без вступлений вроде «а знаете ли вы». "
    "1-3 предложения. Если не уверен в достоверности факта — выбери более безопасный "
    "и точно проверенный, не выдумывай. "
    "ЯЗЫК ОТВЕТА — ТОЛЬКО РУССКИЙ. Даже если источники в поиске на украинском, "
    "английском или любом другом языке — переведи и напиши ответ полностью по-русски. "
    "Ни одного слова на другом языке, никакой смеси языков."
)
...
# Буквы, которых нет в русском алфавите, но которые типичны для украинского —
# если grounded-поиск подтянул источник на другом языке и модель ответила не
# по-русски вопреки системному промпту, это довольно надёжно её выдаёт.
_NON_RUSSIAN_LETTERS = set("іїєґІЇЄҐ")

def _looks_non_russian(text: str) -> bool:
    return any(ch in _NON_RUSSIAN_LETTERS for ch in text)
...
async def get_fun_fact(kind: str, context: str = "") -> str:
    ...
    try:
        ...
        if _looks_non_russian(text):
            logger.warning(f"факт ({kind}) пришёл не на русском, ухожу на фолбэк: {text!r}")
            return random.choice(cfg["fallback"])
        return text
    except Exception:
        return random.choice(cfg["fallback"])
...
def build_check_kb(cid: int, d_str: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="победа ✅",  callback_data=f"save_{cid}_{d_str}_success"),
        InlineKeyboardButton(text="срыв 😔",    callback_data=f"save_{cid}_{d_str}_fail"),
    ]])
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "🎲 факт дня (про животных или исторический) — в 10:00\n"
...
async def process_partner_custom_name(message: Message, state: FSMContext):
    if message.text.lower() in ["отмена", "назад"]:
        await state.clear()
        return await message.answer("отменено", reply_markup=main_menu_keyboard())
    name = message.text.strip()
...
async def save_status(callback: CallbackQuery):
    ...
    if status == DayStatus.fail:
        if has_target_date:
            ...
            await callback.message.answer(
                "похоже на срыв 😔\n\nиспользовать заморозку? стрик сохранится, день уйдёт как пропуск.",
                reply_markup=kb
            )
        else:
            kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def use_freeze(callback: CallbackQuery):
    ...
    await callback.answer("🧊 стрик в безопасности!")
...
async def toggles(callback: CallbackQuery):
    async with async_session_maker() as session:
        ...
        if callback.data == "toggle_silent":
            ...
            toast = "🔕 уведомления без звука" if u.silent_mode else "🔔 уведомления со звуком"
        else:
            u.missed_day_policy = (
                DayStatus.fail if u.missed_day_policy == DayStatus.skip else DayStatus.skip
            )
            toast = "😵 пропущенный день → срыв" if u.missed_day_policy == DayStatus.fail else "⏭ пропущенный день → автопропуск"
        await session.commit()
...
async def daily_fact_task(bot: Bot) -> None:
    """Раз в день, в 10:00 по локальному времени пользователя — один факт,
    тема 50/50 между animal и history (см. pick_fact_kind)."""
    async with async_session_maker() as session:
        now_utc = datetime.now(timezone.utc)
        res = await session.execute(select(User))
        for u in res.scalars():
...
async def main():
    ...
    scheduler.add_job(daily_fact_task,   "interval", minutes=1, args=[bot])
    scheduler.start()
